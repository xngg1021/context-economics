...
| 2–4 smoke, 8–12 pilot, 24–40 holdout pairs | preparable | zero actual provider pairs |
...
| Hermes replay | partial exact-source trace-replay | budget properties and tool boundary methods only |
| Cache × compression | descriptive four-cell analyzer | no real four-arm campaign or cache-control evidence |
| Content-type retention | synthetic literal-presence simulation | no semantic/task-success or real-compressor claim |
...
Executor failure aborts and atomically preserves completed canonical events,
...
partial failure accounting; economic inference remains blocked when failed
...
This does not execute the summary model, prompt rebuild, memory/profile
injection, protected-head lifecycle, session reset or provider transport.
Those fields remain null. Full compressor replay remains outstanding.
...
## Independent attestation contract (design only)

A future verifier must authenticate artifact-directory hash, manifest hash,
receipt hash, provider-evidence hash, scorer-evidence hash, external attester
identity, timezone-aware issuance time and signature/reference against a trust
root outside the runner. No offline trust root or verifier is implemented in
this successor. No self-attestation, caller switch or numerical performance
result can unlock evidence eligibility or production mutation.
